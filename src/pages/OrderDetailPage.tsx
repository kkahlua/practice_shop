...
const OrderDetailPage = () => {
  ...
  const getStatusStepClass = (step: number) => {
    if (!currentOrder) return "bg-gray-300 dark:bg-gray-600";

    const currentStep =
      currentOrder.status === OrderStatus.PAYMENT_COMPLETED
        ? 1
  ...
  const handleCancelOrder = async () => {
    if (!orderId || !currentOrder?.canCancel) return;
    ...
    try {
      ...
      dispatch(
        showToast({ message: "주문이 취소되지 않았습니다", type: "error" })
      );
    } finally {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Order #{currentOrder.id.substring(0, 8)} • 주문 날짜{" "}
            ...
            <div className="flex flex-col items-center z-10">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {currentOrder.status === OrderStatus.SHIPPING
                  ...
                  ? new Date(currentOrder.updatedAt - 5000).toLocaleDateString()
                  : "-"}
              </p>
            </div>
                  ...
                  <li key={index} className="py-4 first:pt-0 last:pb-0">
                    <div className="flex items-start">
                      ...
                      <div className="ml-4 flex-shrink-0 text-right">
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                          1개당 {item.price.toLocaleString("ko-KR")}원
                        </p>
                      </div>
                    </div>
                  </li>
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900 dark:text-white font-medium">
                    {currentOrder.totalAmount.toLocaleString("ko-KR")}원
                  </span>
                </div>
